...
class HERReplayBuffer:
    """
    A simple FIFO experience replay buffer for DQN+HER agents.
    """
...
class HEREpisodeBuffer:
    """
    A simple buffer for each episode implemented with lists.
    """
    ...
    def final_state(self):
        return self.obs2_buf[-1]

    def __iter__(self):
        for o, a, r, o2, d in zip(self.obs_buf, self.act_buf, self.rew_buf, self.obs2_buf, self.done_buf):
            yield {'obs': o, 'act': a, 'rew': r, 'obs2': o2, 'done': d}

    def __len__(self):
        return len(self.obs_buf)


def her_dqn(env_fn, policy=core.MLPQPolicy, policy_kwargs=dict(), double_Q=True,
            seed=0, steps_per_epoch=4000, epochs=100, replay_size=int(1e6), gamma=0.99,
            polyak=0.995, q_lr=1e-3, batch_size=100, start_steps=10000, update_after=1000,
            update_every=50, num_test_episodes=10, eps_start=0.9, eps_end=0.05,
            eps_decay=200, max_ep_len=1000, logger_kwargs=dict(), save_freq=1):
    """
    ...
    q_policy = policy(env.observation_space['observation'], env.action_space, **policy_kwargs)
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            if not double_Q:
                ...
                q_pi_target = q_target.q_net(o2).gather(1, next_state_action.long()).squeeze(-1)
    ...
    def test_agent():
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                full_o, r, d, _ = test_env.step(get_action(o, eps_threshold=0.0))
                o = full_o['observation']
                ep_ret += r
                ep_len += 1
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            for data in iter(episode_buffer):
                ...
                replay_buffer.store(data['obs'], data['act'], new_rew, data['obs2'], data['done'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_double_Q', action='store_true', default=False,
                        help='If true, does not uses double Q-Learning algorithm.')
    ...
    her_dqn(lambda: gym.make(args.env), policy=core.MLPQPolicy,
            policy_kwargs=dict(hidden_sizes=[args.hid] * args.l),
            double_Q=not args.no_double_Q, gamma=args.gamma, seed=args.seed,
