...
import gym
import time
import rlbase.algos.td3.core as core
...
def her_td3(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    """
    Twin Delayed Deep Deterministic Policy Gradient (TD3) with Hindsight Experience Replay (HER).
    This implementation uses the `final` goal resampling scheme of the paper.
    Works with Continuous GoalEnv environments, e.g., `rlbase:BitFlip-v` continuous version
    or Fetch robotics environments.
    ...
    # Experience buffer
    replay_buffer = HERReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size)
    ...
    def test_agent():
        for j in range(num_test_episodes):
            ...
            while not (d or (ep_len == max_ep_len)):
                ...
                full_o, r, d, _ = test_env.step(get_action(o, 0))
                o = full_o['observation']
                ep_ret += r
                ep_len += 1
...
if __name__ == '__main__':
    ...
    her_td3(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
